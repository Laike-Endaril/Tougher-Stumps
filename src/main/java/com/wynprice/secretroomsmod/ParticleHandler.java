...
public class ParticleHandler
{
    ...
    public void onLivingUpdate(LivingUpdateEvent event)
    {
        ...
        if (event.getEntity().isSprinting() && !event.getEntity().isInWater() &&
                event.getEntity().world.getTileEntity(blockpos) instanceof TileEntityInfomationHolder &&
                ((TileEntityInfomationHolder) event.getEntity().world.getTileEntity(blockpos)).getMirrorState() != null)
        {
            event.getEntity().world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, event.getEntity().posX +
                    ...
                    Block.getStateId(((TileEntityInfomationHolder) event.getEntity().world.getTileEntity(blockpos)).getMirrorState()));
        }
    }
}
